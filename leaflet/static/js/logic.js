...
const data = 'static/data/accidents2020.json'
...
d3.json(data).then( function(response) {
  ...
  response.forEach(feature => {
    ...
    const dateArray = feature.Time.$date.split('T');
    let newDate = new Date(dateArray);
    let timeArray = dateArray[1];
    const removeZ = timeArray.split('Z');
    let time = removeZ[0];
    const date = newDate.toDateString();
    const utcHour = newDate.getUTCHours();
    const utcMin = newDate.getUTCMinutes();
    const utcSec = newDate.getUTCSeconds();
    ...
    if (feature.Latitude) {
      ...
      markers.addLayer(L.marker([feature.Latitude, feature.Longitude], {icon: accidentIcon})
        .bindPopup(`<h1>${feature.City}, ${feature.State}</h1> <hr>
                    ...
                    <h3>Severity(1-4) of Accident: ${feature.Severity}</h3> <hr>
    ...
    legend.onAdd = function() {
      ...
      labels = ['<strong>Accident Severity</strong>'],
      categories = [1, 2, 3, 4];
      for (var i=0; i < categories.length; i++) {
        div.innerHTML +=
        labels.push(
          '<p><i class="fas fa-car" style="background:' + getColor(categories[i]) + '"></i> ' +
